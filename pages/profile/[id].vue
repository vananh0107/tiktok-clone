<template>
  <MainLayout>
    ...
    >
      <div class="flex w-[calc(100vw-260px)]">
        ...
        <div class="ml-5 w-full">
          ...
          <button
            ...
            @click="
              message === 'Follow'
                ? handleFollow(route.params.id)
                : handleUnFollow(route.params.id)
            "
          >
            {{ message }}
          </button>
      ...
      <div class="flex items-center pt-4">
        <div class="mr-4">
          <span class="font-bold">{{ $profileStore.follows?.length }}</span>
...
const isFollowed = computed(() => {
  let res = $userStore?.follows?.find((id) => id === route.params.id);
  if ($userStore.id) {
