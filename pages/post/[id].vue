    ...
    >
      ...
      <div class="flex items-center justify-between px-8">
        <div class="flex items-center">
          <NuxtLink
            :href="`/profile/${$generalStore.selectedPost.post.author.id}`"
          >
            <img
              class="rounded-full lg:mx-0 mx-auto h-[38px]"
              width="40"
              :src="$generalStore.selectedPost.post.author.image"
            />
          </NuxtLink>
          <div class="ml-3 pt-0.5">
            <div class="text-[17px] font-semibold">
              {{
                $generalStore.allLowerCaseNoCaps(
                  $generalStore.selectedPost.post.author.displayName
                )
              }}
            </div>
            <div class="text-[13px] -mt-5 font-light">
              ...
              <span class="font-medium">{{
                $generalStore.selectedPost.post.created_at
              }}</span>
            </div>
          </div>
        </div>
        ...
        <Icon
          v-if="$userStore.id === $generalStore.selectedPost.post.author.id"
          @click="deletePost()"
      ...
      <div class="px-8 mt-4 text-sm">
        {{ $generalStore.selectedPost.post.caption }}
      </div>
      ...
      <div class="flex items-center px-8 mt-8">
        <div class="pb-4 text-center flex items-center">
          ...
          <span class="text-xs pl-2 pr-4 text-gray-800 font-semibold">
            {{ $generalStore.selectedPost.favorites?.length }}
          </span>
        </div>
        ...
        >
          <div class="flex items-center relative w-full">
            <NuxtLink :to="`/profile/${comment.author?.id}`">
              <img
                class="absolute top-0 rounded-full lg:mx-0 mx-auto h-[38px]"
                width="40"
            ...
            <div class="ml-14 pt-0.5 w-full">
              ...
              >
                ...
                <Icon
                  v-if="$userStore.id === comment.author?.id"
                  @click="
                    deleteComment($generalStore.selectedPost.post, comment.id)
                  "
                  class="cursor-pointer"
                  name="material-symbols:delete-outline-sharp"
                  size="25"
...
const loopThroughPostsDown = () => {
  setTimeout(() => {
    ...
    for (let i = 0; i < idArrayReversed.length; i++) {
      const objectId = idArrayReversed[i];
      if (objectId.id < route.params.id) {
        router.push(`/post/${objectId.id}`);
        isBreak = true;
        return;
      }
    }
    if (!isBreak) {
      router.push(`/post/${idArrayReversed[0]?.id}`);
    }
  }, 300);
};
...
const loopThroughPostsUp = () => {
  setTimeout(() => {
    ...
    for (let i = 0; i < $generalStore.ids?.length; i++) {
      const objectId = $generalStore?.ids[i];
      if (objectId?.id > route.params.id) {
        router.push(`/post/${objectId?.id}`);
        isBreak = true;
        return;
    ...
    if (!isBreak) {
      router.push(`/post/${$generalStore?.ids[0]?.id}`);
    }
  }, 300);
};

const isLiked = computed(() => {
  let res = $generalStore.selectedPost.favorites?.find(
    (like) => like.user === $userStore.id
  );
  if (res) {
    return true;
  }
...
const deletePost = async () => {
  ...
  try {
    if (res) {
      await $userStore.deletePost($generalStore.selectedPost.post);
...
const addComment = async () => {
  try {
    await $userStore.addComment($generalStore.selectedPost.post, comment.value);
    comment.value = null;
    document.getElementById('Comments').scroll({ top: 0, behavior: 'smooth' });
    await $userStore.updateComments($generalStore.selectedPost.post)
  } catch (error) {
    console.log(error);
  }
};
